...
set.seed(101) 
# Split up the sample
sample <- sample.split(clinical.aorta["lya.decrease"], SplitRatio = 0.80) # SplitRatio = percent of sample==TRUE)
# Training Data
...
# build the model
model <- lm(lya.decrease ~ lya.start,train)
#print(summary(model))
...
pdf("/home/ef2p/Kaggle/lymphopenia/figures/lm0-res.pdf")
print(pl)
dev.off()
